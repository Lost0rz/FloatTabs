...
final class AppCommandControllerTests: XCTestCase {
    func testOnlyExplicitFloatTabsShortcutsAreMatched() {
        XCTAssertEqual(AppCommandController.command(characters: "1", keyCode: 18, modifiers: [.command]), .selectSlot(1))
        XCTAssertEqual(AppCommandController.command(characters: "9", keyCode: 25, modifiers: [.command]), .selectSlot(9))
        XCTAssertEqual(AppCommandController.command(characters: "t", keyCode: 17, modifiers: [.command]), .addWebApp)
        XCTAssertEqual(AppCommandController.command(characters: "l", keyCode: 37, modifiers: [.command]), .quickURL)
        XCTAssertEqual(AppCommandController.command(characters: "+", keyCode: 24, modifiers: [.command, .shift]), .zoomIn)
        XCTAssertEqual(AppCommandController.command(characters: "=", keyCode: 24, modifiers: [.command]), .zoomIn)
        XCTAssertEqual(AppCommandController.command(characters: "-", keyCode: 27, modifiers: [.command]), .zoomOut)
        XCTAssertEqual(AppCommandController.command(characters: "0", keyCode: 29, modifiers: [.command]), .resetZoom)
        XCTAssertEqual(AppCommandController.command(characters: "\t", keyCode: 48, modifiers: [.control]), .nextSlot)
        XCTAssertEqual(AppCommandController.command(characters: "\t", keyCode: 48, modifiers: [.control, .shift]), .previousSlot)
    ...
    func testQuickURLDismissesForEscapeSecondCommandLAndOutsideClickOnly() {
        let window = NSWindow(contentRect: NSRect(x: 0, y: 0, width: 500, height: 500), styleMask: [.borderless], backing: .buffered, defer: false)
        ...
        let commandL = NSEvent.keyEvent(with: .keyDown, location: .zero, modifierFlags: [.command], timestamp: 0, windowNumber: window.windowNumber, context: nil, characters: "l", charactersIgnoringModifiers: "l", isARepeat: false, keyCode: 37)!
        XCTAssertTrue(AppCommandController.shouldDismissQuickURL(for: commandL, overlay: overlay))

        let insideClick = NSEvent.mouseEvent(with: .leftMouseDown, location: NSPoint(x: 150, y: 320), modifierFlags: [], timestamp: 0, windowNumber: window.windowNumber, context: nil, eventNumber: 1, clickCount: 1, pressure: 1)!
        XCTAssertFalse(AppCommandController.shouldDismissQuickURL(for: insideClick, overlay: overlay))

        let outsideClick = NSEvent.mouseEvent(with: .leftMouseDown, location: NSPoint(x: 40, y: 40), modifierFlags: [], timestamp: 0, windowNumber: window.windowNumber, context: nil, eventNumber: 2, clickCount: 1, pressure: 1)!
...
final class MobileRenderingDiagnosticTests: XCTestCase {
    func testMediumWebsiteModeIsMobile() {
        let (_, webView) = makeMobileSurface(width: 430, height: 820)
        XCTAssertEqual(webView.websiteMode, .mobile)
    }

    func testMediumWebViewWidthIs430Probe() {
        let (_, webView) = makeMobileSurface(width: 430, height: 820)
    ...
    func testMediumAttachedWebViewWidthIs390() {
        ...
        guard let webView = WebViewFactory.makeWebView(renderingProfile: rendering) as? FloatTabsWebView else { fatalError("Expected FloatTabsWebView") }
        let container = WebPanelContainerView(frame: NSRect(x: 0, y: 0, width: 430, height: 820))
        container.show(webView: webView)
        let window = NSWindow(contentRect: NSRect(x: 0, y: 0, width: 430, height: 820), styleMask: [.borderless], backing: .buffered, defer: false)
    ...
    private func makeMobileSurface(width: CGFloat, height: CGFloat) -> (WebPanelContainerView, FloatTabsWebView) {
        let rendering = WebRenderingProfile.canonicalDefault.settingWebsiteMode(.mobile)
        guard let webView = WebViewFactory.makeWebView(renderingProfile: rendering) as? FloatTabsWebView else { fatalError("Expected FloatTabsWebView") }
        let container = WebPanelContainerView(frame: NSRect(x: 0, y: 0, width: width, height: height))
        container.show(webView: webView)
        container.layoutSubtreeIfNeeded()
        webView.layoutSubtreeIfNeeded()
